...
export class SPListService {
  ...
  public async getListItems_search_v4(columnName: string, columnValue: string | null, _sp:any | null, otherColumnNames?: string[] | null): Promise<any[]> {
    ...
    let allColumnProperties = [columnName];
    ...
    while (hasMoreResults) {
        let query = SearchQueryBuilder()
          ...
          if(otherColumnNames != null){ 
            query = query.selectProperties(columnName, ...allColumnProperties);
          }
          else { 
        ...
        try {
            ...
            //console.log(results);
            const titles_allProps = results.PrimarySearchResults.filter(x => ((x.Path) && x.Path.indexOf("Lists/"+this.listName)>-1)); 
            const titles = titles_allProps.map(item => item.Title);
